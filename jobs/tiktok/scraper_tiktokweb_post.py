...
CONFIGS_LINK = read_file("configs/product_id_tiktok_video.yaml", FileFormat.YAML)
...
async def build_tiktok_headers(proxy) -> Dict[str, str]:
    ...
    # h["Referer"] = f"https://www.tiktok.com/search?q={quote(keyword)}"
...
async def fetch_product(link_config: str, proxy_url: str, worker_id: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Xác định loại content và ID
        m = re.search(r"/(video|photo)/(\d+)", link_config)
        content_type, content_id = m.groups()
        headers = await build_tiktok_headers(proxy=proxy_url)
        ...
        if content_type == "photo":
            try: 
                ...
                client_args = {"timeout": 15.0, "proxy": proxy_url}
                async with httpx.AsyncClient(**client_args) as client:
                    resp_json = await _fetch_with_retry(client=client, url=complete_url, headers=headers, retries=3)

                if not resp_json:
                        logger.bind(save=True).warning(f"[Worker {worker_id}] Empty response for PHOTO item_id={content_id}")
                        return None
                
                logger.info(f"[Worker {worker_id}] Successfully fetched PHOTO item_id={content_id}")
                return {
                        "scrape_date": TODAY,
                        "share_url": link_config,
                        "data": resp_json
                    }
            except httpx.TimeoutException:
                    logger.bind(save=True).warning(f"[Worker {worker_id}] Timeout while fetching PHOTO id={content_id}")
                    return None
            except httpx.ConnectError as e:
                logger.bind(save=True).error(f"[Worker {worker_id}] Connection error PHOTO id={content_id} -> {e}")
                return None
            except Exception as e:
                logger.bind(save=True).error(f"[Worker {worker_id}] Unexpected error PHOTO id={content_id} -> {e}")
                return None

        # --- VIDEO MODE -----------------------------------------------------
        else:
            client_args = {"timeout": 15.0, "headers": headers, "proxy": proxy_url}
            try: 
                async with httpx.AsyncClient(**client_args) as client:
                ...
                if html:
                    ...
                    if not script_tag:
                        logger.error(f"[Worker {worker_id}] No script tag found product_id={link_config}")
                        return None

                    data = json.loads(script_tag.string)
                
                    match = re.search(r"/(\d+)$", link_config)
                    if match:
                        video_id = match.group(1)
                        safe_filename = f"{video_id}.html"
                    
                    html_path = os.path.join(HTML_DATA_PATH, safe_filename)
                ...
                logger.bind(save=True).error(f"[Worker {worker_id}] Request error product_id={link_config} error={e}")
                return None
    except Exception as e:
        logger.bind(save=True).error(f"[Worker {worker_id}] Unhandled exception url={url} -> {e}")
        return None
...
async def crawl_configs(unprocessed_configs, RAW_DATA_PATH, worker_id=0, batch_size=5):
    ...
    for i in range(0, len(unprocessed_configs), batch_size):
        ...
        tasks = [fetch_product(pid, proxy_url, worker_id) for pid in batch]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for product_id, data_raw in zip(batch, results):
            if not data_raw:
                # Bỏ qua, không lưu
                logger.bind(save=True).warning(f"[Worker {worker_id}] Skip product_id={product_id} vì request thất bại.")
                continue
...
async def main(link_configs = CONFIGS_LINK, max_attempts=10):
    FULL_CONFIG = link_configs
    logger.bind(save=True).info(f"RUN START - total_configs={len(FULL_CONFIG)}")
    processed_configs, unprocessed_configs = await split_processed_configs_tiktok_post(FULL_CONFIG, RAW_DATA_PATH)
